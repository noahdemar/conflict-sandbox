...
for (const demo of DEMOS) {
  ...
  const prev = existsSync(manifestPath)
    ? (JSON.parse(readFileSync(manifestPath, 'utf8')) as { voice: string; items: Record<string, { text: string; spoken?: string; file: string }> })
    : null;
  const items: Record<string, { text: string; spoken?: string; file: string }> = {};
  ...
  for (const k of keyframes) {
    ...
    const unchanged = prev?.voice === VOICE && prev.items[k.id]?.text === k.caption &&
      prev.items[k.id]?.spoken === toSpokenText(k.caption) && existsSync(join(dir, file));
    if (!unchanged) {
      const audio = await tts.generate(toSpokenText(k.caption), { voice: VOICE });
    ...
    items[k.id] = { text: k.caption, spoken: toSpokenText(k.caption), file };
